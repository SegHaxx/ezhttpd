...
func (cw *chunkWriter) writeHeader(p []byte) {
	...
	if !header.has("Date") {
		//dateBuf := bytes.NewBuffer(make([]byte,0,len(TimeFormat)))
		setHeader.date = []byte(time.Now().Format(TimeFormat))
	}
...
func (c *loggingConn) Write(p []byte) (n int, err error) {
	fmt.Printf("%s.Write(%d) = ....", c.name, len(p))
	n, err = c.Conn.Write(p)
	fmt.Printf("%s.Write(%d) = %d, %v", c.name, len(p), n, err)
	return
}

func (c *loggingConn) Read(p []byte) (n int, err error) {
	fmt.Printf("%s.Read(%d) = ....", c.name, len(p))
	n, err = c.Conn.Read(p)
	fmt.Printf("%s.Read(%d) = %d, %v", c.name, len(p), n, err)
	return
}

func (c *loggingConn) Close() (err error) {
	fmt.Printf("%s.Close() = ...", c.name)
	err = c.Conn.Close()
	fmt.Printf("%s.Close() = %v", c.name, err)
	return
}
